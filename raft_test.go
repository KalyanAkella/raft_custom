...
func nextEnts(r *raft, s *storage.EntryStore) (ents []pb.Entry) {
...
func TestProgressResumeByHeartbeatResp(t *testing.T) {
	r := newTestRaft(1, []uint64{1, 2}, 5, 1, newEntryStorage())
	r.becomeCandidate()
	r.becomeLeader()
	r.prs[2].Paused = true
...
func TestLearnerElectionTimeout(t *testing.T) {
	n1 := newTestLearnerRaft(1, []uint64{1}, []uint64{2}, 10, 1, newEntryStorage())
	n2 := newTestLearnerRaft(2, []uint64{1}, []uint64{2}, 10, 1, newEntryStorage())
...
func testVoteFromAnyState(t *testing.T, vt pb.MessageType) {
	for st := StateType(0); st < numStates; st++ {
		r := newTestRaft(1, []uint64{1, 2, 3}, 10, 1, newEntryStorage())
		r.Term = 1
...
func TestDuelingCandidates(t *testing.T) {
	a := newTestRaft(1, []uint64{1, 2, 3}, 10, 1, newEntryStorage())
	b := newTestRaft(2, []uint64{1, 2, 3}, 10, 1, newEntryStorage())
	c := newTestRaft(3, []uint64{1, 2, 3}, 10, 1, newEntryStorage())
	...
	tests := []struct {
		...
		{c, StateFollower, 2, newLog(newEntryStorage(), raftLogger)},
	}
...
func TestDuelingPreCandidates(t *testing.T) {
	cfgA := newTestConfig(1, []uint64{1, 2, 3}, 10, 1, newEntryStorage())
	cfgB := newTestConfig(2, []uint64{1, 2, 3}, 10, 1, newEntryStorage())
	cfgC := newTestConfig(3, []uint64{1, 2, 3}, 10, 1, newEntryStorage())
	cfgA.PreVote = true
	cfgB.PreVote = true
	cfgC.PreVote = true
	a := newRaft(cfgA)
	...
	tests := []struct {
		...
		{c, StateFollower, 1, newLog(newEntryStorage(), raftLogger)},
	}
...
func TestProposal(t *testing.T) {
	...
	for j, tt := range tests {
		...
		wantLog := newLog(newEntryStorage(), raftLogger)
		if tt.success {
			wantLog = &raftLog{
				storage: &MemoryStorage{
					ents: []pb.Entry{{}, {Data: nil, Term: 1, Index: 1}, {Term: 1, Index: 2, Data: data}},
...
func TestPastElectionTimeout(t *testing.T) {
	...
	for i, tt := range tests {
		sm := newTestRaft(1, []uint64{1}, 10, 1, newEntryStorage())
...
func TestRaftFreesReadOnlyMem(t *testing.T) {
	sm := newTestRaft(1, []uint64{1, 2}, 5, 1, newEntryStorage())
	sm.becomeCandidate()
	sm.becomeLeader()
	sm.raftLog.commitTo(sm.raftLog.lastIndex())
...
func TestMsgAppRespWaitReset(t *testing.T) {
	sm := newTestRaft(1, []uint64{1, 2, 3}, 5, 1, newEntryStorage())
	sm.becomeCandidate()
	sm.becomeLeader()
...
func TestAllServerStepdown(t *testing.T) {
	...
	for i, tt := range tests {
		sm := newTestRaft(1, []uint64{1, 2, 3}, 10, 1, newEntryStorage())
		switch tt.state {
		case StateFollower:
...
func TestNonPromotableVoterWithCheckQuorum(t *testing.T) {
	a := newTestRaft(1, []uint64{1, 2}, 10, 1, newEntryStorage())
	b := newTestRaft(2, []uint64{1}, 10, 1, newEntryStorage())
...
func TestLeaderIncreaseNext(t *testing.T) {
	...
	for i, tt := range tests {
		sm := newTestRaft(1, []uint64{1, 2}, 10, 1, newEntryStorage())
...
func TestSendAppendForProgressProbe(t *testing.T) {
	r := newTestRaft(1, []uint64{1, 2}, 10, 1, newEntryStorage())
	r.becomeCandidate()
	r.becomeLeader()
	r.readMessages()
	r.prs[2].becomeProbe()
...
func TestRestoreFromSnapMsg(t *testing.T) {
	...
	sm := newTestRaft(2, []uint64{1, 2}, 10, 1, newEntryStorage())
	sm.Step(m)
...
func TestAddNode(t *testing.T) {
	r := newTestRaft(1, []uint64{1}, 10, 1, newEntryStorage())
	r.pendingConf = true
	r.addNode(2)
	if r.pendingConf {
...
func TestRemoveLearner(t *testing.T) {
	r := newTestLearnerRaft(1, []uint64{1}, []uint64{2}, 10, 1, newEntryStorage())
	r.pendingConf = true
	r.removeNode(2)
	if r.pendingConf {
		t.Errorf("pendingConf = %v, want false", r.pendingConf)
...
func TestPromotable(t *testing.T) {
	...
	for i, tt := range tests {
		r := newTestRaft(id, tt.peers, 5, 1, newEntryStorage())
...
func TestRaftNodes(t *testing.T) {
	...
	for i, tt := range tests {
		r := newTestRaft(1, tt.ids, 10, 1, newEntryStorage())
...
func testCampaignWhileLeader(t *testing.T, preVote bool) {
	cfg := newTestConfig(1, []uint64{1}, 5, 1, newEntryStorage())
	cfg.PreVote = preVote
	r := newRaft(cfg)
	if r.state != StateFollower {
...
func TestCommitAfterRemoveNode(t *testing.T) {
	...
	dir := fmt.Sprintf("%s/node-%d", os.TempDir(), time.Now().UnixNano())
	s, _ := storage.NewEntryStore(dir)
...
func TestTransferNonMember(t *testing.T) {
	r := newTestRaft(1, []uint64{2, 3, 4}, 5, 1, newEntryStorage())
...
func TestNodeWithSmallerTermCanCompleteElection(t *testing.T) {
	n1 := newTestRaft(1, []uint64{1, 2, 3}, 10, 1, newEntryStorage())
	n2 := newTestRaft(2, []uint64{1, 2, 3}, 10, 1, newEntryStorage())
	n3 := newTestRaft(3, []uint64{1, 2, 3}, 10, 1, newEntryStorage())
...
type network struct {
	...
	storage map[uint64]*storage.EntryStore
...
func newNetworkWithConfig(configFunc func(*Config), peers ...stateMachine) *network {
	...
	nstorage := make(map[uint64]*storage.EntryStore, size)
	...
	for j, p := range peers {
		...
		case nil:
			dir := fmt.Sprintf("%s/node-%d", os.TempDir(), time.Now().UnixNano())
			nstorage[id], _ = storage.NewEntryStore(dir)
